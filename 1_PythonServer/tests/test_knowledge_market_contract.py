...
class FakeRemoteClient:
    def __init__(self, payload: dict[str, object] | None = None) -> None:
        self.payload = payload or knowledge_market_index()
...
class FakeArchive:
    ...
    def validate_and_extract(self, *, staging_root: Path, **_kwargs) -> Path:
        target = staging_root / "extracted-knowledge"
...
def test_knowledge_market_settings_use_independent_default_source(tmp_path) -> None:
    path = tmp_path / "knowledge" / "market-settings.json"
    ...
    assert settings.source == DEFAULT_KNOWLEDGE_MARKET_SOURCE
    assert json.loads(path.read_text(encoding="utf-8"))["source"] == (
        DEFAULT_KNOWLEDGE_MARKET_SOURCE
    )


def test_knowledge_market_accepts_empty_catalog_and_uses_cached_network_fallback(tmp_path) -> None:
    payload = knowledge_market_index(projects=[])
    service, remote, *_ = build_knowledge_market_service(tmp_path, payload=payload)

    async def run() -> None:
        first = await service.get_index()
        assert first.kind == "tiance-knowledge-market"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("tiance-knowledge-market", True),
    ],
)
def test_knowledge_market_rejects_wrong_kind_and_duplicate_ids(
    tmp_path,
    kind: str,
    duplicate: bool,
) -> None:
    projects = [knowledge_market_entry()]
    if duplicate:
        projects.append(knowledge_market_entry())
    payload = knowledge_market_index(projects=projects)
    payload["kind"] = kind
    service, *_ = build_knowledge_market_service(tmp_path, payload=payload)

    with pytest.raises(BadRequestError):
        asyncio.run(service.get_index())


def test_knowledge_install_preserves_snapshot_and_rebinds_only_project_identity(tmp_path) -> None:
    template = tmp_path / "knowledge-template"
    conversations = template / ".Tiance" / "conversations"
    memory = template / ".Tiance" / "memory"
    conversations.mkdir(parents=True)
    memory.mkdir(parents=True)
    (template / "notes.md").write_text("knowledge", encoding="utf-8")
    ...
    service, _, project_service, knowledge_root, _ = build_knowledge_market_service(
        tmp_path,
    ...
    assert project.project_kind is ProjectKind.KNOWLEDGE
    assert Path(project.root_path).parent == knowledge_root
    identity = json.loads((Path(project.root_path) / ".Tiance" / "project.json").read_text())
    index = json.loads((Path(project.root_path) / ".Tiance" / "conversations" / "index.json").read_text())
    graph = json.loads((Path(project.root_path) / ".Tiance" / "conversations" / "branch_graph.json").read_text())
    memory_record = json.loads((Path(project.root_path) / ".Tiance" / "memory" / "project_memory.jsonl").read_text())
    origin = json.loads((Path(project.root_path) / ".Tiance" / "market.json").read_text())
    ...
    assert origin["project_kind"] == "knowledge"


def test_knowledge_install_rejects_non_knowledge_category(tmp_path) -> None:
    service, _, project_service, *_ = build_knowledge_market_service(tmp_path)
    category = project_service.create_project_category(
        name="普通项目分类",
        category_kind=ProjectKind.PROJECT,
    )

    with pytest.raises(BadRequestError, match="知识分类"):
        asyncio.run(service.start_install(
            market_project_id="knowledge-sample",
            category_id=category.category_id,
        ))


def test_knowledge_install_rejects_already_installed_market_project(tmp_path) -> None:
    service, _, project_service, *_ = build_knowledge_market_service(tmp_path)
    category = project_service.create_project_category(
        name="重复安装",
        category_kind=ProjectKind.KNOWLEDGE,
    )
    first = asyncio.run(install_and_wait(service, category.category_id))
...
def test_knowledge_install_failure_rolls_back_catalog_and_directory(tmp_path) -> None:
    service, _, project_service, knowledge_root, catalog = build_knowledge_market_service(
        tmp_path,
        creation_fails=True,
    )
    category = project_service.create_project_category(
    ...
    assert not [path for path in knowledge_root.iterdir() if path.is_dir() and path.name != ".market-cache"]
    assert not list((knowledge_root / ".market-cache" / "operations").iterdir())


def test_knowledge_market_close_cancels_download_and_cleans_temporary_files(tmp_path) -> None:
    service, remote, project_service, knowledge_root, _ = build_knowledge_market_service(tmp_path)
...
def build_knowledge_market_service(
    tmp_path: Path,
    *,
    ...
    project_service = ProjectService(
        repository,
        ProjectStorage(projects_root, knowledge_root=knowledge_root),
    )
    ...
    if template_root is None:
        ...
        (template_root / "README.md").write_text("knowledge", encoding="utf-8")
    ...
    service = ProjectMarketApplicationService(
        settings_repository=ProjectMarketSettingsRepository(
            knowledge_root / "market-settings.json",
            default_source=DEFAULT_KNOWLEDGE_MARKET_SOURCE,
        ),
        cache_repository=ProjectMarketCacheRepository(
            knowledge_root / ".market-cache",
        ),
    ...
    service.prepare()
    return service, remote, project_service, knowledge_root, catalog


def knowledge_market_entry() -> dict[str, object]:
    return {
        "id": "knowledge-sample",
        "name": "知识示例",
        "summary": "包含文件、会话、分支和记忆的知识项目。",
        "author": "LikeMirage",
        "version": "1.0.0",
        "updatedAt": "2026-08-02T00:00:00Z",
        "download": {
            "kind": "archive",
            "url": "packages/knowledge-sample.zip",
        },
        "tags": ["knowledge"],
...
def knowledge_market_index(
    *,
    projects: list[dict[str, object]] | None = None,
) -> dict[str, object]:
    return {
        "schemaVersion": 1,
        "kind": "tiance-knowledge-market",
        "name": "Tiance Knowledge",
        "updatedAt": datetime.now(UTC).isoformat(),
        "defaultRef": "main",
        "projects": [knowledge_market_entry()] if projects is None else projects,
    }
