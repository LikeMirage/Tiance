...
router = APIRouter(prefix="/projects/market", tags=["project-market"])


@router.get("/settings", response_model=ProjectMarketSettingsResponse)
def read_project_market_settings() -> ProjectMarketSettingsResponse:
    return get_project_market_application_service().get_settings()


@router.put("/settings", response_model=ProjectMarketSettingsResponse)
def update_project_market_settings(
...
async def read_project_market_index() -> ProjectMarketIndexResponse:
    return await get_project_market_application_service().get_index()


@router.post("/connect", response_model=ProjectMarketIndexResponse)
async def connect_project_market(
    payload: ProjectMarketConnectRequest,
) -> ProjectMarketIndexResponse:
    return await get_project_market_application_service().connect(payload.source)


@router.post("/restore-default", response_model=ProjectMarketIndexResponse)
async def restore_default_project_market() -> ProjectMarketIndexResponse:
    return await get_project_market_application_service().restore_default_source()


@router.get("/previews/{market_project_id}", response_class=FileResponse)
async def read_project_market_preview(market_project_id: str) -> FileResponse:
    path = await get_project_market_application_service().get_preview_path(
        market_project_id
    )
...
async def install_project_market_project(
    market_project_id: str,
    payload: ProjectMarketInstallRequest,
) -> ProjectMarketInstallOperation:
    return await get_project_market_application_service().start_install(
...
def read_project_market_operation(operation_id: str) -> ProjectMarketInstallOperation:
    return get_project_market_application_service().get_operation(operation_id)
