...
class Md2DocxConverter:
    ...
    def __init__(
        ...
        *,
        ...
        self.media = MediaWriter(self.doc, base_path, self._warnings, render_budget)
        self.inline = InlineWriter(fonts, self.formulas, self.media, self.notes)
        self.tables = TableWriter(self.doc, fonts, self.inline)
        self.lists = NativeListWriter(self.doc, fonts, self.inline)
    ...
    def _setup_document(self, page_orientation: str, page_size: str) -> None:
        ...
        word_page_layout.apply_document_page_layout(self.doc, page_orientation, page_size)
        ...
        *,
        ...
        if line.startswith(opener) and line.endswith(closer) and len(line) > len(opener) + len(closer):
            ...
            if end is None:
                ...
                formatting.normalize_paragraph(paragraph)
    ...
    def _add_heading(self, line: str) -> None:
        ...
        formatting.normalize_heading_paragraph(paragraph, level)
        heading_size = formatting.heading_font_size(level)
    ...
    def _add_paragraph(self, text: str) -> None:
        paragraph = self.doc.add_paragraph()
        formatting.normalize_paragraph(paragraph, first_line=True)
        self.inline.write(paragraph, text, font_size=Pt(12))
        formatting.style_runs(paragraph.runs, self.fonts)
