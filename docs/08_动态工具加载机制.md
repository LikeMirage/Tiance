...
权限检查也沿同一调用结构执行。`execute_dynamic_tool` 和目标工具都是调用图中的普通节点，执行器逐节点读取各自真实 Schema、参数和权限表，再以“禁止高于询问、询问高于允许”的规则合并结果。包装工具不按名称获得豁免，目标工具也不会借用包装工具的权限表。需要询问时，界面仍显示模型发起的外层工具调用，但确认详情保留实际触发权限的工具名和参数名。
