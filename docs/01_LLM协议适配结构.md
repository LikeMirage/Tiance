...
因此社区可以共同补齐小供应商和特殊模型，而不必每次修改聊天核心代码。市场包不包含 `credentials.json`。首次安装使用包内完整供应商定义和模型模板；已有供应商更新时只替换 `manifest.json`、`provider-rules.json`、`model-rules.json`，不会修改 `provider.json`、密钥、模型、云端模型缓存、本地分类或 `.Tiance` 运行数据。

兼容更新必须保持同一个供应商 ID、供应商数据结构和 `profileId`。如果新版本要求改变 Profile 身份、重置用户配置或依赖当前天策尚不支持的程序能力，就不是规则更新：它必须提高最低天策版本，或使用新的供应商 ID 作为同名新包发布。完整制作与发布流程见《12_供应商适配包制作与发布》。

天策没有“官方供应商”或拥有特殊更新权限的“内置供应商”。初始数据目录里已有的供应商只是随安装包提前放入的普通供应商包，与用户后来创建或从任意市场来源安装的包使用相同目录、稳定 ID、配置合同和更新规则。市场来源只决定从哪里读取索引与下载包，不决定供应商等级。

供应商目录和配置读取都采用单包隔离：一个供应商文件不完整时，列表会返回其它正常供应商，并单独报告损坏供应商；模型目录也只跳过该损坏项，不再让全部供应商看板和模型选择一起失效。旧数据缺少后来新增的 `reasoningReplayMode` 字段时，启动迁移只补入原有默认值，不覆盖密钥、地址、协议或其它用户配置。
